...
**Parallel by default when work is parallel-safe.** Inline execution is for narrow, single-focus tasks. When a task spans multiple files, modules, or concerns — fan out. Doing multi-module work serially inline is slow and produces shallow results because each piece is done without the others in context. Agents own a scoped piece, run in parallel, write their findings to verify-state.json, and the parent synthesizes. Speed and depth both improve.

**Routing is a start decision, not a fallback.** Do not start inline and switch to agents when it gets too big. That is not routing — the context is already polluted, the work is already half-done, and the agents get a worse brief. Route at the start. If the initial classification turns out to be wrong after reading the code, re-route before writing any code — not after.
